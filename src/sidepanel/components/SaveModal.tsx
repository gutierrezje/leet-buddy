    ...
    <Dialog open={open} onOpenChange={(o) => !o && onCancel()}>
      <DialogContent>
        ...
        <div className='flex items-start flex-col w-full'>
          <div className="text-xl font-semibold border-b w-full mb-2">Problem: {problem.title}</div>
          {problem.difficulty && (
            <div className="text-base mb-2">
              Difficulty:{' '}
              <div className='flex items-center gap-1'>
              <div className={cn("w-2 h-2 rounded-full", difficultyColorsBg[problem.difficulty])}></div>
              <span className={cn('text-sm', difficultyColorsText[problem.difficulty])}>
                {problem.difficulty}
              </span>
              </div>
            </div>
          ...
          <div className='text-base mb-2'>
            Tags:{' '}
            <div className="text-sm text-muted-foreground">{problem.tags?.join(', ')}</div>
          </div>
